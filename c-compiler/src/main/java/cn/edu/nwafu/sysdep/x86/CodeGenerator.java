...
import cn.edu.nwafu.ir.*;
import cn.edu.nwafu.sysdep.CodeGeneratorOptions;
import cn.edu.nwafu.utils.AsmUtils;
import cn.edu.nwafu.utils.ErrorHandler;
import cn.edu.nwafu.utils.ListUtils;

import java.util.*;

public class CodeGenerator implements cn.edu.nwafu.sysdep.CodeGenerator,
    ...
    private void loadGOTBaseAddress(AssemblyCode file, cn.edu.nwafu.sysdep.x86.Register reg) {
        file.call(PICThunkSymbol(reg));
        file.add(imm(GOT), reg);
    }
    // #@@}

    private cn.edu.nwafu.sysdep.x86.Register GOTBaseReg() {
        return bx();
    }
    ...
    private Symbol PICThunkSymbol(cn.edu.nwafu.sysdep.x86.Register reg) {
     ...
     */
    // #@@range/PICThunk{
    private void PICThunk(AssemblyCode file, cn.edu.nwafu.sysdep.x86.Register reg) {
    ...
    class StackFrameInfo {
        List<cn.edu.nwafu.sysdep.x86.Register> saveRegs;
        long lvarSize;
        long tempSize;
    ...
    private List<cn.edu.nwafu.sysdep.x86.Register> usedCalleeSaveRegisters(AssemblyCode body) {
        List<cn.edu.nwafu.sysdep.x86.Register> result = new ArrayList<cn.edu.nwafu.sysdep.x86.Register>();
        for (cn.edu.nwafu.sysdep.x86.Register reg : calleeSaveRegisters()) {
    ...
    private List<cn.edu.nwafu.sysdep.x86.Register> calleeSaveRegistersCache = null;

    private List<cn.edu.nwafu.sysdep.x86.Register> calleeSaveRegisters() {
        if (calleeSaveRegistersCache == null) {
            List<cn.edu.nwafu.sysdep.x86.Register> regs = new ArrayList<cn.edu.nwafu.sysdep.x86.Register>();
            for (RegisterClass c : CALLEE_SAVE_REGISTERS) {
                regs.add(new cn.edu.nwafu.sysdep.x86.Register(c, naturalType));
            }
    ...
    private void prologue(AssemblyCode file,
                          List<cn.edu.nwafu.sysdep.x86.Register> saveRegs, long frameSize) {
        file.push(bp());
        file.mov(sp(), bp());
        for (cn.edu.nwafu.sysdep.x86.Register reg : saveRegs) {
    ...
    // #@@range/epilogue{
    private void epilogue(AssemblyCode file, List<cn.edu.nwafu.sysdep.x86.Register> savedRegs) {
        for (cn.edu.nwafu.sysdep.x86.Register reg : ListUtils.reverse(savedRegs)) {
            file.virtualPop(reg);
        }
        file.mov(bp(), sp());
        file.pop(bp());
    ...
    private void compileBinaryOp(Op op, cn.edu.nwafu.sysdep.x86.Register left, Operand right) {
     ...
     */
    // #@@range/loadConstant{
    private void loadConstant(Expr node, cn.edu.nwafu.sysdep.x86.Register reg) {
    ...
    // #@@range/loadVariable{
    private void loadVariable(Var var, cn.edu.nwafu.sysdep.x86.Register dest) {
        if (var.memref() == null) {
            cn.edu.nwafu.sysdep.x86.Register a = dest.forType(naturalType);
    ...
    // #@@range/loadAddress{
    private void loadAddress(Entity var, cn.edu.nwafu.sysdep.x86.Register dest) {
        if (var.address() != null) {
    ...
    // #@@range/reg_dsls2{
    private cn.edu.nwafu.sysdep.x86.Register ax() { return ax(naturalType); }
    private cn.edu.nwafu.sysdep.x86.Register al() { return ax(Type.INT8); }
    private cn.edu.nwafu.sysdep.x86.Register bx() { return bx(naturalType); }
    // #@@}
    private cn.edu.nwafu.sysdep.x86.Register cx() { return cx(naturalType); }
    private cn.edu.nwafu.sysdep.x86.Register cl() { return cx(Type.INT8); }
    private cn.edu.nwafu.sysdep.x86.Register dx() { return dx(naturalType); }

    // #@@range/reg_dsls1{
    private cn.edu.nwafu.sysdep.x86.Register ax(Type t) {
        return new cn.edu.nwafu.sysdep.x86.Register(RegisterClass.AX, t);
    }

    private cn.edu.nwafu.sysdep.x86.Register bx(Type t) {
        return new cn.edu.nwafu.sysdep.x86.Register(RegisterClass.BX, t);
    }
    // #@@}

    private cn.edu.nwafu.sysdep.x86.Register cx(Type t) {
        return new cn.edu.nwafu.sysdep.x86.Register(RegisterClass.CX, t);
    }

    private cn.edu.nwafu.sysdep.x86.Register dx(Type t) {
        return new cn.edu.nwafu.sysdep.x86.Register(RegisterClass.DX, t);
    }

    private cn.edu.nwafu.sysdep.x86.Register si() {
        return new cn.edu.nwafu.sysdep.x86.Register(RegisterClass.SI, naturalType);
    }

    private cn.edu.nwafu.sysdep.x86.Register di() {
        return new cn.edu.nwafu.sysdep.x86.Register(RegisterClass.DI, naturalType);
    }
    ...
    private cn.edu.nwafu.sysdep.x86.Register sp() {
        return new cn.edu.nwafu.sysdep.x86.Register(RegisterClass.SP, naturalType);
    }
    ...
    private IndirectMemoryReference mem(cn.edu.nwafu.sysdep.x86.Register reg) {
        return new IndirectMemoryReference(0, reg);
    }

    private IndirectMemoryReference mem(long offset, cn.edu.nwafu.sysdep.x86.Register reg) {
        return new IndirectMemoryReference(offset, reg);
    }

    private IndirectMemoryReference mem(Symbol offset, cn.edu.nwafu.sysdep.x86.Register reg) {
        return new IndirectMemoryReference(offset, reg);
    }
    // #@@}
    ...
    // #@@range/load{
    private void load(MemoryReference mem, cn.edu.nwafu.sysdep.x86.Register reg) {
        as.mov(mem, reg);
    }
    // #@@}

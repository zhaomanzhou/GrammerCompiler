...
import cn.edu.nwafu.entity.DefinedFunction;
import cn.edu.nwafu.entity.DefinedVariable;
import cn.edu.nwafu.entity.Entity;
import cn.edu.nwafu.entity.LocalScope;
import cn.edu.nwafu.exception.JumpError;
import cn.edu.nwafu.exception.SemanticException;
...
class IRGenerator implements ASTVisitor<Void, Expr>
{
    ...
    private cn.edu.nwafu.asm.Type asmType(Type t) {
        if (t.isVoid()) return int_t();
        return cn.edu.nwafu.asm.Type.get(t.size());
    }

    private cn.edu.nwafu.asm.Type varType(Type t) {
        if (! t.isScalar()) {
            return null;
        }
        return cn.edu.nwafu.asm.Type.get(t.size());
    ...
    private cn.edu.nwafu.asm.Type int_t() {
        return cn.edu.nwafu.asm.Type.get((int)typeTable.intSize());
    }

    private cn.edu.nwafu.asm.Type size_t() {
        return cn.edu.nwafu.asm.Type.get((int)typeTable.longSize());
    }

    private cn.edu.nwafu.asm.Type ptr_t() {
        return cn.edu.nwafu.asm.Type.get((int)typeTable.pointerSize());
    }

    private cn.edu.nwafu.asm.Type ptrdiff_t() {
        return cn.edu.nwafu.asm.Type.get((int)typeTable.longSize());
    }
